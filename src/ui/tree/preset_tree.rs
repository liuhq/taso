//! Based on https://github.com/ynqa/promkit/blob/main/promkit/src/preset/tree.rs commit 61c4ad859db8d7f05a34f23757912fd6bf43454c

use crate::ui::tree::{
    hack::TodoStyle, preset_evaluate, widget_node::Node, widget_tree,
};
use promkit::{
    Prompt, Signal, async_trait,
    core::{
        PaneFactory,
        crossterm::{
            self,
        ...
        self.renderer = Some(SharedRenderer::new(
            Renderer::try_new_with_panes(
                [
                    ...
                    (Index::Tip, self.title.create_pane(size.0, size.1)),
...
impl Tree {
    ...
    pub fn new(root: Node) -> Self {
        Self {
            renderer: None,
            evaluator: |event, ctx| {
                Box::pin(preset_evaluate::default(event, ctx))
            },
            title: text::State {
                style: ContentStyle {
            ...
            tree: widget_tree::State::new(root),
        }
    }
    ...
    /// Sets the style for symbol.
    pub fn symbol_style(mut self, style: ContentStyle) -> Self {
        self.tree.symbol_style = style;
        self
    }

    /// Sets the style for active (currently selected) items.
    pub fn active_item_style(mut self, style: TodoStyle) -> Self {
        self.tree.active_item_style = style;
        self
    }

    /// Sets the style for inactive (not currently selected) items.
    pub fn inactive_item_style(mut self, style: TodoStyle) -> Self {
            ...
            Some(renderer) => {
                renderer
                    .update([
                        ...
                        (Index::Tip, self.title.create_pane(width, height)),
